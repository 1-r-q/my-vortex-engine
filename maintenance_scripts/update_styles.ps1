...
foreach ($item in $styles.GetEnumerator()) {
    $path = "e:\ai\완성\code zero\my-vortex-engine\.vitepress\theme\components\apps\" + $item.Key
    $content = Get-Content -Path $path -Raw -Encoding UTF8
    if (-not $content) { Write-Host "Checking $path... not found" }
    else {
        if ($content -notmatch "Mobile Only Styles") {
            $newContent = $content + "`n" + $item.Value
            Set-Content -Path $path -Value $newContent -Encoding UTF8
            Write-Host "Updated $path"
        } else {
            Write-Host "Skipped $path (already has styles)"
        }
    }
}